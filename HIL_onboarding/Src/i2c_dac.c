...
#define MCP4728_VOUTA_DAC1 0U
#define MCP4728_VOUTA_DAC0 0U

#define MCP4728_VOUTB_DAC1 0U
#define MCP4728_VOUTB_DAC0 1U
...
static HAL_StatusTypeDef buildOutputFrame(uint16_t code, I2cDacFrame_t *frame)
{
    ...
    // frame->bytes[0] = (1 << 7U) | (1 << 6U) | (0 << 5U) | (0 << 4U) | (MCP4728_A2 << 3U) | (MCP4728_A1 << 2U) | (MCP4728_A0 << 1U) | 0U;
    frame->bytes[0] = (MCP4728_C2 << 7U) | (MCP4728_C1 << 6U) | (MCP4728_C0 << 5U) | (MCP4728_W1 << 4U) | (MCP4728_W0 << 3U) | (MCP4728_VOUTA_DAC1 << 2U) | (MCP4728_VOUTA_DAC0 << 1U) | MCP4728_UDAC;
    frame->bytes[1] = (MCP4728_VREF << 7U) | (MCP4728_PD1 << 6U) | (MCP4728_PD0 << 5U) | (MCP4728_GX << 4U) | ((code >> 8U) & 0x0FU);
    frame->bytes[2] = code & 0xFFU;

    frame->bytes[3] = (MCP4728_C2 << 7U) | (MCP4728_C1 << 6U) | (MCP4728_C0 << 5U) | (MCP4728_W1 << 4U) | (MCP4728_W0 << 3U) | (MCP4728_VOUTB_DAC1 << 2U) | (MCP4728_VOUTB_DAC0 << 1U) | MCP4728_UDAC;
    frame->bytes[4] = (MCP4728_VREF << 7U) | (MCP4728_PD1 << 6U) | (MCP4728_PD0 << 5U) | (MCP4728_GX << 4U) | ((code >> 8U) & 0x0FU);
    frame->bytes[5] = code & 0xFFU;
    frame->length = 6U;
...
static HAL_StatusTypeDef transmitFrame(const I2cDacFrame_t *frame)
{
    ...
    return HAL_I2C_Master_Transmit(&hi2c1, I2C_DAC_ADDRESS7 << 1U, frame->bytes, frame->length, I2C_DAC_TIMEOUT_MS);
